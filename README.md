...
- `Bert_inference.ipynb`: load a trained model and run predictions on new text inputs.
- `Bert_inference_with_highlight.ipynb`: inference notebook with attention-based highlighting to interpret model outputs.
- `outputs/`: saved artifacts such as logs, metrics, or exported models (if generated by the notebooks).
...
## Inference workflows
- After training, open `Bert_inference.ipynb` to load your saved model and score new inputs.
- If you want interpretability, use `Bert_inference_with_highlight.ipynb` and supply the same tokenizer/model artifacts from training.
